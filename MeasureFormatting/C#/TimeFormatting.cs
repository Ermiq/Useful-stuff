public static void GetTimeFromSeconds(float secondsTotal, out int s, out int m, out int h, out int d, out int y)
{
	...
	secondsTotal -= s;
	// of nothing left then it was less than 1 minute (45 - 0 = 45).
	if (secondsTotal < 60)
		return;
	...
	// divide by 3600 => how many hours remain
	// % 24 => what will remain after splitting to days (24 hours)
	h = (int)(secondsTotal / 3600 % 24);
	// every while hour takes 3600 secs from the total
	secondsTotal -= h * 3600;
